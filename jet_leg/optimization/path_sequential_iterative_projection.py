...
class PathIterativeProjection:
    ...
    def setup_path_iterative_projection(self, params):
        ...
        proj, eq, ineq, actuation_polygons, isIKoutOfWorkSpace = self.compDyn.setup_iterative_projection(
            params, False)
    ...
    def optimize_direction_variable_constraint(self, lp, vdir, solver=GLPK_IF_AVAILABLE):
        ...
        lp_q[-2] = -vdir[0]
        lp_q[-1] = -vdir[1]
        x = solve_lp(lp_q, lp_Gextended, lp_hextended,
                     lp_A, lp_b, solver=solver)
        tempSolution = x[-2:]
...
#        normals = params.getNormals()
        ...
        lp, actuation_polygons, isOutOfWorkspace = iterProj.setup_path_iterative_projection(
            params)
    ...
    def find_intersection(self, vertices_input, desired_direction, comWF):
        ...
        for i in range(0, len(vertices_input)):
            ...
            if new_point:
                ...
                while new_point is False:
                    desired_com_line = self.line(
                        comWF, comWF+desired_direction+np.array([random()*0.01, random()*0.01, 0.0]))
                    new_point = self.two_lines_intersection(
                        desired_com_line, actuation_region_edge)
                    intersection_points = np.vstack(
                        [intersection_points, new_point])
            ...
            if np.abs(desired_direction[0] - comWF[0]) > epsilon:
                alpha_com_x_line = (
                    new_point[0] - comWF[0])/(desired_direction[0] - comWF[0])
            else:
                alpha_com_x_line = 1000000000.0

            if np.abs(desired_direction[1] - comWF[1]) > epsilon:
                alpha_com_y_line = (
                    new_point[1] - comWF[1])/(desired_direction[1] - comWF[1])
            else:
    ...
    def find_vertex_along_path(self, params, desired_direction, tolerance=0.05, max_iteration_number=10):
        ...
        while (np.amax(np.abs(increment)) > tolerance) and (while_iter < max_iteration_number):
            ...
            if polygon:
                ...
                new_p, all_points = self.find_intersection(
                ...
                if np.size(new_p, 0) == 0:
                    ...
                    stackedIncrements = np.vstack(
                        [stackedIncrements, increment])
