...
for LH_x in np.arange(-0.8, -0.3, 0.1):
    for LH_y in np.arange(0.1, 0.4, 0.1):
        for dir_y in np.arange(-0.2, 0.0, 0.2):
            ...
            newLimitPoint = pathIP.find_vertex_along_path(
                constraint_mode, contacts, comWF, desired_direction)

            for_iter += 1
            comTrajectoriesToStack = np.vstack(
                [comTrajectoriesToStack, newLimitPoint])
            optimizedVariablesToStack = np.vstack(
                [optimizedVariablesToStack, np.array([LH_x, LH_y, dir_y])])

max_motion_indices = np.unravel_index(np.argsort(
    comTrajectoriesToStack[:, 0], axis=None), comTrajectoriesToStack[:, 0].shape)
...
plt.plot(comTrajectoriesToStack[:, 0],
         comTrajectoriesToStack[:, 1], 'g^', markersize=20)
plt.plot(comTrajectoriesToStack[max_motin_index, 0],
         comTrajectoriesToStack[max_motin_index, 1], 'y^', markersize=25)
h1 = plt.plot(contacts[0:nc, 0], contacts[0:nc, 1],
              'ko', markersize=15, label='feet')

plt.plot(optimizedVariablesToStack[:, 0],
         optimizedVariablesToStack[:, 1], 'ko', markersize=15)
plt.plot(optimizedVariablesToStack[max_motin_index, 0],
         optimizedVariablesToStack[max_motin_index, 1], 'yo', markersize=25)
plt.xlim(-0.9, 0.5)
plt.ylim(-0.7, 0.7)
plt.legend()
plt.show()
