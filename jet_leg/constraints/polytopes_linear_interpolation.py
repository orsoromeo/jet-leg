...
class polytopesLinearInterpolation:
    def __init__(self, robot_kinematics, comp_dynamics):
        ...
        self.maxRetractionFeetPos = np.vstack(
            (LF_foot, RF_foot, LH_foot, RH_foot))
        LF_foot = np.array([0.3, 0.2, -0.4])
        RF_foot = np.array([0.3, -0.2, -0.4])
        LH_foot = np.array([-0.3, 0.2, -0.4])
        RH_foot = np.array([-0.3, -0.2, -0.4])
        self.maxExtensionFeetPos = np.vstack(
            (LF_foot, RF_foot, LH_foot, RH_foot))
        trunk_mass = 85.
        mu = 0.5
        ...
        n1 = np.transpose(np.transpose(
            math.rpyToRot(0.0, 0.0, 0.0)).dot(axisZ))  # LF
        n2 = np.transpose(np.transpose(
            math.rpyToRot(0.0, 0.0, 0.0)).dot(axisZ))  # RF
        n3 = np.transpose(np.transpose(
            math.rpyToRot(0.0, 0.0, 0.0)).dot(axisZ))  # LH
        n4 = np.transpose(np.transpose(
            math.rpyToRot(0.0, 0.0, 0.0)).dot(axisZ))  # RH
        ...
        C, d, isIKoutOfWorkSpace, forcePolytopes = comp_dyn.constr.getInequalities(
            params)
