    ...
    def getInequalities(self, params, saturate_normal_force=False):
        ...
        for j in stanceIndex:
            ...
            if constraint_mode[j] == 'ONLY_FRICTION':
                Ctemp, d_cone = self.frictionConeConstr.linearized_cone_halfspaces_world(
                    params.useContactTorque, friction_coeff, normals[j, :], contact_torque_lims)
...
#                Ctemp = np.dot(constraints_local_frame, rotationMatrix.T)
            ...
            if constraint_mode[j] == 'ONLY_ACTUATION':
                Ctemp, d_cone, leg_polygon, isIKoutOfWorkSpace = self.forcePolytopeConstr.compute_actuation_constraints(
                    j, joint_torque_lims, params.useContactTorque, contact_torque_lims, rpy)
            ...
            if constraint_mode[j] == 'FRICTION_AND_ACTUATION':
                C1, d1, leg_polygon, isIKoutOfWorkSpace = self.forcePolytopeConstr.compute_actuation_constraints(
                    j, joint_torque_lims, params.useContactTorque, contact_torque_lims, rpy)
                leg_actuation_polygon[j] = leg_polygon
                C2, d2 = self.frictionConeConstr.linearized_cone_halfspaces_world(
                    params.useContactTorque, friction_coeff, normals[j, :], contact_torque_lims)
            ...
            if isIKoutOfWorkSpace is False:
                ...
                self.currentLegForcePolytope.setVertices(
                    leg_actuation_polygon[j])
                forcePolytopes.forcePolytope[j].setVertices(
                    leg_actuation_polygon[j])
            else:
        ...
        return C, d, isIKoutOfWorkSpace, forcePolytopes, q, knees_pos, hips_pos
