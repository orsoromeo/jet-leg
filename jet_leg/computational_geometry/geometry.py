...
class Geometry(Math):
    ...
    def clockwise_sort(self, polygon):
        ...
        for j in range(0, vertices_number):
            angle[0, j] = np.arctan2(
                polygon[j, 0] - centroidX, polygon[j, 1] - centroidY)
    ...
    def flip_normal_inwards(self, facets, vertices):
        ...
        # sum all the rows but the last one (the last vertex is a duplicate of the first one)
        centroidX = np.sum(vertices[:-1, 0])
        centroidX = centroidX/float(vertices_number)
        centroidY = np.sum(vertices[:-1, 1])
        centroidY = centroidY/float(vertices_number)
        # the centroid is always inside the polygon, therefore its distance from the edges has to be always negative
