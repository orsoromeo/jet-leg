...
class Kinematics:
    ...
    def compute_xy_IK(self, x, x_dot, z, z_dot):
        ...
        hfe2foot = np.sqrt(
            np.square(footPosHAA[0]) + np.square(footPosHAA[1])) - haa2hfeLength
        # add the x component
        ...
        cos_arg = (upperLegLength * upperLegLength + lowerLegLength * lowerLegLength -
                   hfe2foot * hfe2foot) / (2 * upperLegLength * lowerLegLength)
        q[2] = - M_PI + np.arccos(cos_arg[0])  # LF KFE
        q[5] = - M_PI + np.arccos(cos_arg[0])  # RF KFE
        cos_arg = (np.square(upperLegLength) + np.square(hfe2foot) -
                   np.square(lowerLegLength)) / (2 * upperLegLength * hfe2foot)
        # it should be footPosHFE(rbd::X)/hfe2foot but footPosHFE(rbd::X) = footPosHAA(rbd::X)
        sin_arg = footPosHAA[0] / hfe2foot
        ...
        cos_arg = (upperLegLength * upperLegLength + hfe2foot * hfe2foot -
                   lowerLegLength * lowerLegLength) / (2 * upperLegLength * hfe2foot)
        # it should be footPosHFE(rbd::X)/hfe2foot but footPosHFE(rbd::X) = footPosHAA(rbd::X)
        sin_arg = footPosHAA[0, 2] / hfe2foot
