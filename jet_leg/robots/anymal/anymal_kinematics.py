...
class anymalKinematics():
    def __init__(self, anymal_type):
        self.PKG = os.path.dirname(os.path.abspath(
            __file__)) + '/../../../resources/urdfs/anymal/'
    ...
    def footInverseKinematicsFixedBase(self, foot_pos_des, frame_name):
        ...
        anymal_q0 = np.vstack(
            [-0.1, 0.7, -1., -0.1, -0.7, 1., 0.1, 0.7, -1., 0.1, -0.7, 1.])
        q = anymal_q0
        eps = 0.05
        IT_MAX = 200
        DT = 1e-1
    ...
    def fixedBaseInverseKinematics(self, feetPosDes, stance_idx):
        ...
        are_stance_feet_in_workspace = np.full(
            (1, number_of_stance_feet), False, dtype=bool)[0]
        previous_flag = True
        for k in np.arange(0, number_of_stance_feet):
            stance_leg_id = stance_idx[k]
            are_stance_feet_in_workspace[k] = self.leg_ik_success[int(
                stance_leg_id)]
