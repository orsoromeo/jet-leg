...
class hyqrealKinematics():
    def __init__(self):
        ...
        self.PKG = os.path.dirname(os.path.abspath(
            __file__)) + '/../../../resources/urdfs/hyqreal/'
    ...
    def footInverseKinematicsFixedBase(self, foot_pos_des, frame_name):
        ...
        q0 = np.vstack([-0.1, 0.7, -1., -0.1, -0.7, 1.,
                       0.1, 0.7, -1., 0.1, -0.7, 1.])
        q = q0
        eps = 0.005
        IT_MAX = 200
    ...
    def fixedBaseInverseKinematics(self, feetPosDes):
        ...
        q_LF, self.LF_foot_jac, err = self.footInverseKinematicsFixedBase(
            f_p_des, self.urdf_foot_name_lf)

        f_p_des = np.array(feetPosDes[2, :]).T
        q_LH, self.LH_foot_jac, err = self.footInverseKinematicsFixedBase(
            f_p_des, self.urdf_foot_name_lh)

        f_p_des = np.array(feetPosDes[1, :]).T
        q_RF, self.RF_foot_jac, err = self.footInverseKinematicsFixedBase(
            f_p_des, self.urdf_foot_name_rf)

        f_p_des = np.array(feetPosDes[3, :]).T
        q_RH, self.RH_foot_jac, err = self.footInverseKinematicsFixedBase(
            f_p_des, self.urdf_foot_name_rh)

        '''please NOTICE here the alphabetical order of the legs in the vector q: LF -> LH -> RF -> RH '''
        q = np.vstack([q_LF, q_LH, q_RF, q_RH])
        legJacs = np.vstack(
            [self.LF_foot_jac, self.LH_foot_jac, self.RF_foot_jac, self.RH_foot_jac])
        return q, legJacs
