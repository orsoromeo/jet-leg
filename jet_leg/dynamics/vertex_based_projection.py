...
class VertexBasedProjection():
    ...
    def convex_hull(self, input_matrix):  # requires 6XN
        ...
        hull = scipy.spatial.ConvexHull(
            input_matrix_t, qhull_options="QbB")  # is pX6
        # get the matrix
            ...
            for i in range(j+1, n_a):
                ...
                if (not np.array_equal(lambda1, lambda2)):
                    ...
                    if (np.abs(np.dot(normal_plane, (lambda1-lambda2))) > 1e-04):
                        ...
                        alpha = np.dot(normal_plane, (lambda1 - point_plane)) / \
                            np.dot(normal_plane, (lambda1-lambda2))
    ...
    def project(self, params, robot_name):
        ...
        if params.constraintMode[0] == 'ONLY_ACTUATION':
            ...
            vertices_1 = np.transpose(
                constr.forcePolytopeConstr.computeLegActuationPolygon(J_LF, torque_lims[0]))
            vertices_2 = np.transpose(
                constr.forcePolytopeConstr.computeLegActuationPolygon(J_RF, torque_lims[1]))
            vertices_3 = np.transpose(
                constr.forcePolytopeConstr.computeLegActuationPolygon(J_LH, torque_lims[2]))

        elif params.constraintMode[0] == 'ONLY_FRICTION':

            n1, n2, n3 = (math_lp.normalize(n) for n in [n1, n2, n3])
            R1, R2, R3 = (math_lp.rotation_matrix_from_normal(n)
                          for n in [n1, n2, n3])
        ...
        pointsfZ, points_num = self.compute_section_points(
            w123_hull, np.array([0, 0, 0, 0, 0, 1]), np.array([0, 0, 0, 0, 0, mg]))
        ...
        pointsfY, points_num = self.compute_section_points(
            pointsfZhull, np.array([0, 0, 0, 0, 1]), np.array([0, 0, 0, 0, 0]))
        # to avoid that convex hull complains because they are complanar remove the fz dimension
        pointsfYhull = self.convex_hull(pointsfY[0:4, :])

        pointsfX, points_num = self.compute_section_points(
            pointsfYhull, np.array([0, 0, 0, 1]), np.array([0, 0, 0, 0]))
        # to avoid that convex hull complains because they are complanar remove the fz dimension
        pointsfXhull = self.convex_hull(pointsfX[0:3, :])

        pointstZ, points_num = self.compute_section_points(
            pointsfXhull, np.array([0, 0, 1]), np.array([0, 0, 0]))
        # to avoid that convex hull complains because they are complanar remove the fz dimension
...
#
        ...
        if params.constraintMode[0] == 'ONLY_ACTUATION':
            points2d = self.project_points(
                pointstZhull, mg)  # cimpute com points
        elif params.constraintMode[0] == 'ONLY_FRICTION':
            points2d = self.project_points(
                pointsfZhull, mg)  # cimpute com points

        vertices2d = np.transpose(points2d)
        chull = scipy.spatial.ConvexHull(vertices2d)
        print("Closed form algorith: --- %s seconds ---" %
              (time.time() - start_t))
