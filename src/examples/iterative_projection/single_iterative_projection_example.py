...
LF_foot = np.array([0.2, 0.2, -0.3])
RF_foot = np.array([0.2, -0.2, -0.3])
LH_foot = np.array([-0.2, 0.2, -0.3])
RH_foot = np.array([-0.2, -0.2, -0.3])
...
for j in range(0,nc): # this will only show the contact positions and normals of the feet that are defined to be in stance
    ...
    if isConfigurationStable:
        ...
        b = Arrow3D([contactsWF[idx, 0], contactsWF[idx, 0] + grf[0] / force_scaling_factor],
                    ...
                    [contactsWF[idx, 2], contactsWF[idx, 2] + grf[2] / force_scaling_factor], **arrow_prop_dict)
        ax.add_artist(b)
    else:
    ...
    a = Arrow3D([contactsWF[idx,0], contactsWF[idx,0]+normals[idx,0]/10], [contactsWF[idx,1], contactsWF[idx,1]+normals[idx,1]/10],[contactsWF[idx,2], contactsWF[idx,2]+normals[idx,2]/10], **arrow_prop_dict)
