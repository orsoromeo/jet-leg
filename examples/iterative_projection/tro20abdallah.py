...
for j in np.arange(0, 4):
    contactsWF[j, :] = np.add(
        np.dot(W_R_B, copy.copy(contactsBF[j, :])), comWF)
...
'''
IP_points, force_polytopes, IP_computation_time = comp_dyn.iterative_projection_bretl(
    params)
...
plt.scatter(shoulder_position_WF[0, 0],
            shoulder_position_WF[0, 1], s=250, c='k')
plt.scatter(shoulder_position_WF[1, 0],
            shoulder_position_WF[1, 1], s=250, c='k')
plt.plot(shoulder_position_WF[0, 0], shoulder_position_WF[0,
         1], 'ko', markersize=15, label='stance feet')
h2 = plotter.plot_polygon(np.transpose(IP_points),
                          '--b', 5, 'Improved Feasible Region')
...
'''CoM will be plotted in green if it is stable (i.e., if it is inside the feasible region)'''
if isPointFeasible:
    plt.plot(point2check[0], point2check[1], 'go', markersize=15,
             markeredgecolor='k', label='CoM projection', zorder=10)
    Color = 'g'
else:
    plt.plot(point2check[0], point2check[1], 'ro', markersize=15,
             markeredgecolor='k', label='CoM projection', zorder=10)
    Color = 'r'

plt.plot(zmp[0], zmp[1], 'go', markersize=15,
         markeredgecolor='k', label='ZMP', zorder=10)
...
plt.annotate("CoM acceleration",
             ...
             xytext=(point2check[0] + comWF_lin_acc[0]/acc_scaling_factor - 0.1,
                     point2check[1] + comWF_lin_acc[1]/acc_scaling_factor - 0.01),
